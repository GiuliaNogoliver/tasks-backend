pipeline {
    ...
    stages {
        ...
        stage('API Test'){
            steps {
                dir('api-test') {
                    git credentialsId: 'github_login', url: 'https://github.com/GiuliaNogoliver/tasks-api-test'
                    bat 'mvn test'
                }
            }
        }
        ...
        stage('Deploy Front'){
            steps {
                ...
                bau 'mvn clean package'
                deploy adapters: [tomcat9(credentialsId: 'tomcat_login', path: '', url: 'http://localhost:8001/')], contextPath: 'tasks-frontend', war: 'target/tasks-frontend.war'
            }
        }
    }
}
